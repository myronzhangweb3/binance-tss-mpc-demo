...
import (
	...
	"binance-tss-mpc-server/pkg/chainclients/evm"
...
func (s *ObserverSuite) TestObserverDeckStorage(c *C) {
	...
	// Test Case 1: tx observed in mempool, then non-mempool, then final
	{
		...
		// 3. Transaction finalized (LeaderSalt == FinaliseHeight)
		observedTxFinal := common.ObservedTx{
			...
			FinaliseHeight: finalHeight, // This makes it final (LeaderSalt == FinaliseHeight)
			ObservedPubKey: vault1,
		}
	...
	// Test Case 2: tx observed non-mempool, then final
	{
		...
		// 2. Transaction finalized (LeaderSalt == FinaliseHeight)
		observedTxFinal := common.ObservedTx{
			Tx: common.Tx{
				ID:          txID,
				Chain:       chain,
	...
	// Test Case 3: tx observed final immediately
	{
		...
		// Simulate final observation in Thorchain (already final since LeaderSalt == FinaliseHeight)
