...
func (s *GetConfigTestSuite) Test_GetConfigFromFile_InvalidPath() {
	_, err := tss_config.GetConfigFromFile("invalid", &tss_config.Config{})
...
func (s *GetConfigTestSuite) Test_GetConfigFromENV() {
   ...
   "blockConfirmations":2
	...
	// load from Env
	cnf, err := tss_config.GetConfigFromENV(&tss_config.Config{ChainConfigs: []map[string]interface{}{{
		"id":                 1,
...
func (s *GetConfigTestSuite) Test_SharedConfigLengthMismatch() {
   ...
   "blockConfirmations":2
	...
	// load from Env
	_, err := tss_config.GetConfigFromENV(&tss_config.Config{ChainConfigs: []map[string]interface{}{{
		"id":                 1,
		"blockConfirmations": 5,
		"gasLimit":           500,
	}}})
...
type ConfigTestCase struct {
	name       string
	inConfig   tss_config.RawConfig
	shouldFail bool
	errorMsg   string
	outConfig  tss_config.Config
}
...
func (s *GetConfigTestSuite) Test_GetConfigFromFile() {
	testCases := []ConfigTestCase{
		...
		{
			name: "set default values in tss_config",
			inConfig: tss_config.RawConfig{
				RelayerConfig: relayer.RawRelayerConfig{
	...
	for _, t := range testCases {
		s.Run(t.name, func() {
			...
			conf, err := tss_config.GetConfigFromFile("test.json", &tss_config.Config{
				ChainConfigs: []map[string]interface{}{
					{
						"id": 1,
					},
