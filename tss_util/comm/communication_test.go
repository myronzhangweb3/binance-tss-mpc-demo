...
type CommunicationIntegrationTestSuite struct {
	...
	testHosts          []host.Host
	testCommunications []comm2.Communication
...
func (s *CommunicationIntegrationTestSuite) TestCommunication_BroadcastMessage_SubscribersGotMessage() {
	firstSubChannel := make(chan *comm2.WrappedMessage)
	s.testCommunications[0].Subscribe(s.testSessionID, comm2.CoordinatorPingMsg, firstSubChannel)

	go func() {
		msg := <-firstSubChannel
		s.Equal("1", msg.SessionID)
		s.Equal(comm2.CoordinatorPingMsg, msg.MessageType)
		s.Equal(s.testHosts[2].ID(), msg.From)
	}()

	secondSubChannel := make(chan *comm2.WrappedMessage)
	s.testCommunications[1].Subscribe(s.testSessionID, comm2.CoordinatorPingMsg, secondSubChannel)
...
func (s *CommunicationIntegrationTestSuite) TestCommunication_BroadcastMessage_ErrorOnSendingMessageToExternalHost() {
	...
	externalHost, _ := p2p2.NewHost(privKeyForHost, topology, p2p2.NewConnectionGate(topology), uint16(4005))
...
func (s *CommunicationIntegrationTestSuite) TestCommunication_BroadcastMessage_StopReceivingMessagesAfterUnsubscribe() {
	...
	firstSubChannel := make(chan *comm2.WrappedMessage)
	firstSubID := s.testCommunications[0].Subscribe(s.testSessionID, comm2.CoordinatorPingMsg, firstSubChannel)
 ...
 */
func InitializeHostsAndCommunications(numberOfActors int, protocolID protocol.ID) ([]host.Host, []comm2.Communication) {
	topology := &topology.NetworkTopology{
		Peers: []*peer.AddrInfo{},
	}
	...
	// create test hosts
	for i := 0; i < numberOfActors; i++ {
		newHost, _ := p2p2.NewHost(privateKeys[i], topology, p2p2.NewConnectionGate(topology), uint16(4000+i))
	...
	var testCommunications []comm2.Communication
	for i := 0; i < numberOfActors; i++ {
		com := p2p2.NewCommunication(
			testHosts[i],
			protocolID,
		)
