...
import (
	...
	"testing"
	"time"
	elector2 "tss-demo/tss_util/comm/elector"
...
func (s *BullyTestSuite) SetupIndividualTest(c BullyTestCase) ([]elector2.CoordinatorElector, peer.ID, peer.ID, []host.Host, peer.IDSlice) {
	s.mockController = gomock.NewController(s.T())
	var testHosts []host.Host
	allowedPeers := peer.IDSlice{}
	var testBullyCoordinators []elector2.CoordinatorElector
	...
	for i := 0; i < numberOfTestHosts; i++ {
		...
		newHost, _ := p2p2.NewHost(privateKeys[i], topology, connectionGate, uint16(4000+s.portOffset+i))
	...
	for i := 0; i < numberOfTestHosts; i++ {
		...
		if !c.isLeaderActive && testHosts[i].ID() == initialCoordinator {
			...
			b := elector2.NewBullyCoordinatorElector(s.testSessionID, testHosts[i], relayer.BullyConfig{
