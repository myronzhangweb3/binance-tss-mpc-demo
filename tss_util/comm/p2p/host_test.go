...
func (s *HostTestSuite) TestHost_NewHost_Success() {
	...
	host, err := p2p2.NewHost(
		privKey,
		topology,
		p2p2.NewConnectionGate(topology),
		2020,
	)
	s.Nil(err)
	s.NotNil(host)
...
func (s *HostTestSuite) TestHost_NewHost_InvalidPrivKey() {
	host, err := p2p2.NewHost(
		...
		p2p2.NewConnectionGate(&topology.NetworkTopology{}),
		2020,
	)
	s.Nil(host)
	s.NotNil(err)
...
func (s *LoadPeersTestSuite) SetupTest() {
	...
	host, err := p2p2.NewHost(privKey, topology, p2p2.NewConnectionGate(topology), 2020)
	if err != nil {
		panic(err)
	}
	s.host = host
...
func (s *LoadPeersTestSuite) Test_LoadPeers_RemovesOldAndSetsNewPeers() {
	...
	p2p2.LoadPeers(s.host, []*peer.AddrInfo{newP1, newP2})
