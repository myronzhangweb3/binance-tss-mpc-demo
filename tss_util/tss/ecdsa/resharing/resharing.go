...
func (r *Resharing) Run(
	...
	oldParties := common2.PartiesFromPeers(startParams.OldSubset)
	oldCtx := tss.NewPeerContext(oldParties)
	newParties := r.sortParties(common2.PartiesFromPeers(r.Host.Peerstore().Peers()), oldParties)
...
func (r *Resharing) StartParams(readyPeers []peer.ID) []byte {
	oldSubset := common2.PeersIntersection(r.key.Peers, r.Host.Peerstore().Peers())
...
func (r *Resharing) validateStartParams(params startParams) error {
	...
	if len(r.key.Peers) != 0 && !slices.Equal(params.OldSubset, common2.PeersIntersection(r.key.Peers, r.Host.Peerstore().Peers())) {
...
func (r *Resharing) sortParties(parties tss.SortedPartyIDs, oldParties tss.SortedPartyIDs) tss.SortedPartyIDs {
	...
	oldPeers, _ := common2.PeersFromParties(oldParties)
