...
func (ks *ECDSAKeyshareStore) StoreKeyshare(keyshare ECDSAKeyshare) error {
	address := ethcrypto.PubkeyToAddress(*keyshare.Key.ECDSAPub.ToBtcecPubKey().ToECDSA())
	f, err := os.OpenFile(fmt.Sprintf(ks.path, address), os.O_RDWR|os.O_CREATE|os.O_TRUNC, 0755)
	if err != nil {
		return err
	}
	defer f.Close()
