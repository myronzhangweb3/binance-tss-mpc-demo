...
func (s *KeySign) toLocalTSSSigner(poolPubKey string, tasks []*tssKeySignTask) {
	...
	tssMsg.LeaderSalt = blockHeight / 20 * 20

	s.logger.Info().Msgf("msgToSign to tss Local node PoolPubKey: %s, Messages: %+v, leader salt: %d", tssMsg.PoolPubKey, tssMsg.Messages, tssMsg.LeaderSalt)
