...
func (t *TssServer) KeyResharing(req keyresharing.Request) (keyresharing.Response, error) {
	t.tssKeyGenLocker.Lock()
	defer t.tssKeyGenLocker.Unlock()
	status := common.Success
	msgID, err := t.requestToMsgId(req)
	if err != nil {
		return keyresharing.Response{}, err
	}

	resharing := keyresharing.NewTssKeyResharing(
	...
	keygenMsgChannel := resharing.GetTssKeyGenChannels()
	...
	blameMgr := resharing.GetTssCommonStruct().GetBlameMgr()
	joinPartyStartTime := time.Now()
	onlinePeers, leader, errJoinParty := t.joinParty(msgID, req.Version, req.LeaderSalt, req.Keys, len(req.Keys)-1, sigChan)
	...
	t.logger.Info().Msg("joinParty succeeded, key resharing party formed")
	...
	if err != nil {
		return keyresharing.Response{}, fmt.Errorf("fail to get local keygen state: %w", err)
	}
	...
	k, err := resharing.ResharingNewKey(localStateItem, req)
	keygenTime := time.Since(beforeKeygen)
	if err != nil {
		t.tssMetrics.UpdateKeyGen(keygenTime, false)
		blameNodes := *blameMgr.GetBlame()
		t.logger.Error().Err(err).Msgf("failed to resharing key, blaming: %+v", blameNodes.BlameNodes)
		return keyresharing.NewResponse("", "", common.Fail, blameNodes), err
	} else {
	...
	return keyresharing.NewResponse(
		newPubKey,
		addr,
		status,
		blameNodes,
