...
import (
	...
	"binance-tss-mpc-server/tss/go-tss/monitor"
)
...
func (t *TssServer) joinParty(msgID, version string, leaderSalt int64, participants []string, threshold int, sigChan chan string) ([]peer.ID, string, error) {
	...
	if oldJoinParty {
		...
		return t.partyCoordinator.JoinPartyWithLeader(msgID, leaderSalt, peersIDStr, threshold, sigChan)
	}
}
